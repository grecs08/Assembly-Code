...
LDR R2, =my_var     // Load the address of 'my_var' into R2
LDR R2, [R2]        // Load the value of 'my_var' into R2

// Create a mask to check for the 6th, 17th, and 30th bits
MOV R3, #0x00000000     // Initialize mask to all zeroes
ORR R3, R3, #0x0040     // Set 6th bit to 1
ORR R3, R3, #0x20000    // Set 17th bit to 1
ORR R3, R3, #0x40000000 // Set 30th bit to 1

// Check if the 6th, 17th, and 30th bits are 1 in the value of R2
AND R2, R2, R3          // Apply the mask to the value of R2
CMP R2, R3              // Compare the masked value to the original mask
